...
export default function CashBank() {
  const data = [
    { id: 1, desc: 'Pemasukan 30-Hari Mendatang', color: 'green' },
    { id: 2, desc: 'Pengeluaran 30-Hari Mendatang', color: 'red' },
    { id: 3, desc: 'Saldo Kas', color: 'blue' },
    { id: 4, desc: 'Saldo Kartu Kredit', color: 'blue' },
  ];

  return (
    <div className="wrap-cash-bank h-screen overflow-auto items-center ">
      <div className="kontener-cash-bank mx-auto mb-10">
        <div  className="border-b-2 lg:border-b-4 border-black py-3 px-2 ">
          <div className="inline xs:flex justify-between items-center">
            ...
            <div className="flex xs:inline justify-end xs:justify-normal">
              <button className="py-2 px-3 border-2 rounded-md border-slate-300 text-center">+ Buat Akun Baru</button>
            </div>
          </div>
        </div>
        <div className="mt-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 xl:gap-2">
            {data.map((d, index) => (
                <div key={index} className={`border-${d.color}-800 border-t-4`}>
                  <div className={`bg-${d.color}-500 flex space-x-1 xl:space-x-2 items-center justify-between px-2 h-[65px]`}>
                    <p>{d.desc} (IDR)</p>
                    <div className={`bg-${d.color}-800 px-1 text-white rounded-md text-center`}>0</div>
                  </div>
        ...
        <div  className="border-b-2 lg:border-b-4 border-black py-5 px-2 mt-5">
            ...
            <div className="flex xs:inline justify-end lg:justify-normal space-x-2">
              <div className="inline items-center xs:flex justify-end md:justify-normal space-y-2 space-x-2 xs:space-y-0">
                <div className="flex justify-end xs:justify-normal space-x-1">
                  <button className="py-2 px-3 border-2 rounded-md border-slate-300 text-center">+ Buat Transaksi</button>
                </div>
                <div className="flex justify-end xs:justify-normal space-x-1">
                  <button className="py-2 px-3 border-2 rounded-md border-slate-300 text-center">Peraturan Rekonsiliasi</button>
                </div>
              </div>
            </div>
          </div>
